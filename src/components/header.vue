        ...
        >
            ...
            <template #footer>
                <span class="dialog-footer">
                    ...
                    <el-button type="primary" @click="handleChangeUser"
                    >确认</el-button
                    >
                </span>
            </template>
...
import { checkPassword, validatePassword } from '@/utils/validation.ts'
...
const handleChangeUser = () => {
    if (name.value === '') {
        return global.$message.error('请输入账号')
    }
    global.$http.put('/me', {
        name: name.value
    }).then((result: any)=> {
        if (result.result === 200) {
            global.$message.success('修改用户名成功,跳转至登录页面')
            setTimeout(()=> {
                $useRouter.push('/')
            }, 1000)
        }
...
const handleChangeUserPass = (formEl: FormInstance | undefined) => {
    ...
    formEl.validate((valid) => {
        if (valid) {
            ...
            global.$http.put('/me/password', userPasswordForm).then((result: any)=> {
                if (result.result === 200) {
                    global.$message.success('修改密码成功,将跳转至登录页面')
