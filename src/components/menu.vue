            ...
            <img class="logo-imag" src="@/assets/images/Bitmap.png" alt="">
                <ul class="sidebar-scrollbar-list">
                    <li v-for="(item, index) in sidebarData" :key="index" @click.stop="handleClickRouter(item)">
                        <div class="sidebar-router-child" :class="{ 'router-link-position': $useRoute.path.includes(item.path) }">
                            <div class="sidebar-router sidebar-router-title">
                                <span class="sidebar-nav-name">{{item.name}}</span>
                            </div>
                        </div>
                        <div class="sidebar-scrollbar-children" v-if="item.children.length > 0" >
                            <ul>
                                <li v-for="(row, indexTwo) in item.children" :key="indexTwo" @click.stop="handleClickRouter(row)">
                                    {{row.name}}
                                </li>
                            </ul>
                        </div>
...
const sidebarData: any = [
    ...
    { name: '系统', path: "/system", icon: '', children: [
