...
<template>
    <div class="com-view-components">
        ...
        <template v-if="form.init">
            <el-popover ref="popoverRef" :virtual-ref="viewRef" :width="config.width" :placement="config.placement"
                trigger="click" virtual-triggering popper-class="view-components-popover" @show="handleGetOnePageData">
                <div class="com-index view-components" :style="'height:' + config.height + 'px'">
                    <div class="com-head">
                        <div class="com-head-content com-hc-bg">
                            ...
                            <div class="com-breadcrumb">
                                <el-input v-model="paramsConfig.likeName" :placeholder="config.searchPlaceholder"
                                    clearable @clear="handleDefaultGet" />
                                <el-button :icon="Search" :disabled="paramsConfig.likeName == ''"
                                    @click="handleDefaultGet"></el-button>
                    ...
                    <div class="com-main">
                        <div class="com-table">
                            <el-table :data="form.data" border height="100%" @sort-change="handleSortChange"
                                v-loading="form.loading" element-loading-text="Loading..."
                                element-loading-background="rgba(0, 0, 0, 0.7)">
                                <el-table-column type="index" label="No." show-overflow-tooltip width="60"
                                    :index="typeIndex" />
                                <el-table-column v-for="item in config.showTableColumn" :key="item.prop"
                                    :prop="item.prop" :label="item.label" show-overflow-tooltip />
                            </el-table>
                        </div>
                    </div>
                    <div class="com-footer" v-if="form.data.length > 0">
                        <el-pagination v-model:currentPage="paramsConfig.page" v-model:page-size="paramsConfig.limit"
                            :page-sizes="[10, 20, 50, 100]" layout="total, sizes, prev, pager, next, jumper"
                            :total="form.total" @size-change="handleSizeChange" @current-change="handleCurrentChange" />
                    </div>
                </div>
            </el-popover>
            <i ref="viewRef" :id="form.popover" class="iconfont" :class="config.iconfont" :title="config.iconTitle"
                v-click-outside="onClickOutside"></i>
        </template>
        <!-- 默认渲染图标 -->
        <i v-else class="iconfont" :class="config.iconfont" @click.stop="initLoad" :title="config.iconTitle"></i>
    </div>
</template>
