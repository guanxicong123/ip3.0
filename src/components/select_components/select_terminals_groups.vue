...
const defaultConfig = {
  ...
  showTerminalsGroupsColumn: [
    ...
    { column: "key", text: "No.", style: { width: "15%" } },
    { column: "name", text: proxy.$t("Group name"), style: { width: "55%" } },
    {
      column: "list",
...
const changeTerminalsVolumeConfig = {
  showTerminalsColumn: [
    ...
    },
    {
      ...
      text: proxy.$t("Terminal volume"),
      style: { width: "20%" },
    },
  ],
  showTerminalsGroupsColumn: [
    { column: "key", text: "No.", style: { width: "15%" } },
    { column: "name", text: proxy.$t("Group name"), style: { width: "40%" } },
    {
      column: "list",
      text: proxy.$t("Terminal list"),
      style: { width: "25%" },
...
let config = reactive<any>({
  ...
  amplifierTitle: proxy.$t("Power partition"), // 八分区的标题
    ...
    .then((result) => {
      if (result.data) {
        ...
        if (isArray(result.data)) {
          handleAssemblyDataStructureTerminals(result.result);
  ...
  await GroupsService.getAllGroups({})
    .then((result) => {
      if (result.data) {
        // 组装数据结构
        if (isArray(result.data)) {
          handleAssemblyDataStructureGroups(result.result);
...
// 处理分组组装数据结构
const handleAssemblyDataStructureGroups = (data: any[]) => {
  for (let index = 0; index < data.length; index++) {
    const item = data[index];
    item.task_terminal_vol = parentData.changeTerminalsVolume || 0;
    item.is_lock = 0;
